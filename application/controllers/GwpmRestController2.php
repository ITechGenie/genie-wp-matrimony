...
class GwpmRestController extends WP_REST_Controller {
    ...
    function GwpmRestController() {
        appendLog('GwpmRestController.New Object created !!!') ;
    }
    
    function getUserLoginPreference() {
        appendLog('Before:  ' . $this->_userLoginPreference ) ;
        if (!isset($this->_userLoginPreference)) {
            $this->_userLoginPreference = get_option( GWPM_USER_LOGIN_PREF );
            appendLog('inside getUserLoginPreference: ' . $this->_userLoginPreference) ;
        }
    ...
    public function register_routes() {
        ...
        register_rest_route( $namespace, '/' . $base, array(
                array(
                        ...
                        'callback'        => array( $this, 'get_gwpm_informations' ),
                        'permission_callback' => array( $this, 'get_gwpm_access_info' ),
                        'args'            => array(
                               'operation'=>'GET_GWPM_INFO'
                        ),
                ),
                array(
        ...
        register_rest_route( $namespace, '/' . $base . '/search', array(
                'methods'         => WP_REST_Server::READABLE,
     ...
     */
    public function get_gwpm_informations( $request ) {
        ...
        $data['gwpm_oauth1.0a_setup'] = $oauth10a;
        $resp = new GwpmRestResponseVO($data) ;
        appendLog($resp) ;
    ...
    public function get_gwpm_access_info( $request ) {
        ...
        $data = array();
        $data['gwpm_user_preference'] = $this->getUserLoginPreference();
